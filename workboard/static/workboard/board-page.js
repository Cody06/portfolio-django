document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".archive-btns").forEach(function (button) {
...
function close_save_edit_title() {
  window.onclick = function (event) {
    ...
    if (
      event.target &&
      !event.target.matches("#edit-board-btn") &&
  ...
  Array.prototype.forEach.call(containers, (container) => {
    container.addEventListener("dragover", (event) => {
      ...
      if (afterElement == null) {
        // no following-card, drop it at the end of the column
        container.appendChild(draggable);
      } else {
        // drop the card before the next card (or between two cards)
...
function getDragAfterElement(container, mouse_pos_y) {
  ...
  ).element; // .element to reduce only the element and not also the offset
}
...
function close_dropdown() {
  ...
  window.onclick = function (event) {
    if (!event.target.matches(".dropbtn")) {
      // if user clicks outside the dropdown
      ...
      if (columns.length == 0) {
        ...
        for (let i = 0; i < columns.length; i++) {
          ...
          fetch(`/workboard/cards/${columns[i]["id"]}`) // add all the existing cards
            .then((response) => response.json())
            .then((cards) => {
              // cards holds an array of JSON data
...
function is_board_archived(bool) {
  ...
  if (bool) {
    ...
    document
      .querySelectorAll("button:not(.archive-btns)")
...
function add_col_form() {
  ...
  document
    .querySelector("#create-col-form")
    .addEventListener("submit", save_col);
}
...
function save_col(event) {
  ...
  fetch(`/workboard/columns/${BOARD_ID}`, {
    // send the column name to the server
    method: "POST",
    body: JSON.stringify({
    ...
    .then((response) => response.json())
    .then((column) => {
      // Return the added column so we can have the 'id'
...
function col_options(col_id, action) {
  ...
  if (action == "edit") {
    ...
    document
      .querySelector("#edit-col-form")
      .addEventListener("submit", function () {
        event.preventDefault(); // prevent page refresh
        const EDITED_COL_NAME = document.querySelector(
          "#edit-col-name-input"
        ).value;
        fetch(`/workboard/col-options/${col_id}`, {
          ...
          .then((response) => response.json())
          .then((column) => {
            // update the html column
            document.querySelector(`#col-header-${col_id}`).innerHTML =
              EDITED_COL_NAME; // update the col header
            close_form(); // close the form
  ...
  if (action == "delete") {
    ...
    if (confirm(`This will delete the column: ${COL_NAME}`)) {
      fetch(`/workboard/col-options/${col_id}`, {
        // delete the column from the server
        method: "PUT",
        body: JSON.stringify({
          action: "delete",
        }),
...
function add_card(col_id) {
  ...
  if (document.querySelector("#add-card-div") == null) {
    // if add-card-input is not shown, show it
    ...
    // add-card-input exists, hide it (user pressed 2nd time on '+' button)
...
function create_card_HTML(cards, index) {
  ...
  if (index == -1) {
    // card's variables are from user's input (received index -1)
    card_id = cards["id"];
    ...
    // card's variables are from JSON array received from database
    ...
    document
      ...
      .addEventListener("submit", function () {
        ...
        fetch(`/workboard/cards/${card_id}`, {
          ...
          .then((response) => response.json())
          .then((card) => {
            document.querySelector(`#card-text-${card_id}`).innerHTML =
              NEW_NOTE;
            close_form();
            document
              .querySelector("#edit-card-form")
...
function empty_board_or_not_btn() {
  if (document.querySelector(".col-div") == null) {
    // if we don't have columns on the page
